...
class ComicsViewModel(private val comicsRepository: ComicsRepository): ViewModel() {
    ...
    private val _comicList = mutableStateListOf<Comic>()                                            //è una mutable list, quando la lista viene modificata, Compose rileva i cambiamenti e aggiorna automaticamente l'interfaccia utente.
    ...
    fun getLatestComicsByCharId(charId: String) {
        ...
        viewModelScope.launch {
            try{
                val comicResponse = comicsRepository.getComicsByCharId_api(offset)                   //viene fatta una chiamata alla api specificando l'offset
                val comics = comicResponse.body()?.comicData?.results                  //prendo il corpo della  risposta che contiene i dati
    ...
    fun getComicsByIsbn(isbn: String) {
        ...
        viewModelScope.launch {
            try{
                val comicResponse = comicsRepository.getComicsByIsbn_api(isbn)                   //viene fatta una chiamata alla api specificando l'offset
                val comics = comicResponse.body()?.comicData?.results                  //prendo il corpo della  risposta che contiene i dati
    ...
    fun getLatestComic() {
        _comicList.clear()
            viewModelScope.launch {
                try {
                    val comicResponse = comicsRepository.getLatestComic_api()                   //viene fatta una chiamata alla api specificando l'offset
                    val comics = comicResponse.body()?.comicData?.results                  //prendo il corpo della  risposta che contiene i dati
    ...
    fun getComicByName(name: String) {
        _comicList.clear()

        viewModelScope.launch {
            try{
                val comicResponse = comicsRepository.getComicsByName_api(name, 0)                   //viene fatta una chiamata alla api specificando l'offset
                val comics = comicResponse.body()?.comicData?.results                  //prendo il corpo della  risposta che contiene i dati
